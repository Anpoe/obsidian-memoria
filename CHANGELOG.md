...
## v2.5.0（2026-08-28 · 笔记归档）

### 新增

- 卡片菜单新增“归档 / 取消归档”，归档状态保存在插件数据中，不修改原 Markdown
- 侧栏“视图”分组新增“归档”入口，集中查看和恢复已归档笔记
- 全部笔记、置顶、收藏、日期、标签和自定义检索式默认隐藏已归档笔记
- 编辑、改时间、置顶/收藏和删除笔记时，会同步维护归档状态

---

## v2.4.4（2026-08-17 · 待办与置顶检索修复）

### 修复

- “有已完成待办”只有在笔记内所有待办都完成时才会匹配
- 开启置顶检索豁免后，置顶笔记不受自定义检索式条件限制，选中检索式时始终显示
- 修复自定义检索式与“全部笔记”同时高亮，以及点击“全部笔记”后检索式仍高亮的问题

---

## v2.4.3（2026-08-17 · 检索式与移动端标签体验优化）

### 新增

- 设置页新增“检索式排除置顶笔记”，开启后自定义检索式统一排除置顶笔记，即使其他条件命中

### 优化

- 移动端标签建议改为从当前 `#标签` 光标位置向上展开
- 标签建议最多展示 30 项，并支持手指上下拖动浏览
- “待办”视图同时保留未完成与已完成任务，勾选状态不再改变卡片归类

---

## v2.4.2（2026-08-17 · 移动端标签与任务交互修复）

### 修复

- 修复移动端标签推荐在输入法弹出、文本滚动或短暂失焦时闪退的问题
- 标签推荐固定使用 Memoria 数据集中的标签，不再混入 Vault 全局标签
- 标签列表跟随可视区域变化，并显示在移动输入卡片上方以避开输入法
- 修复任务勾选写回 Markdown 后整条时间线重绘造成的闪屏
- 引用块内任务状态统一纳入未完成与已完成任务检测

---

## v2.4.0（2026-08-10 · 界面与数据报告精修）

### 新增

- 设置页新增“专注 / 均衡 / 宽阔”三档桌面内容宽度，输入区与笔记流可按阅读习惯调整
- 数据报告新增星期分布，并以主题色圆点呈现一周记录节奏

### 优化

- 宠物五维属性条改为跟随 Obsidian 主题色，发送按钮增加更清晰的视觉落点
- 24 小时分布升级为全宽细线节律图，补充高峰时段、峰值数量、占比和时间刻度
- 月度分布改为轻盈窄柱，增加月份之间的留白与统一基线
- 统一输入区、笔记流、控件和面板的宽度、圆角、焦点与响应式细节

### 修复

- 修复部分 Obsidian 环境中 SVG 兼容问题导致统计页后续模块无法显示的问题
- 数据报告各模块改为独立容错，单个图表异常不再阻断其他统计内容

---

## v2.4.0-beta.5（2026-08-10 · 月度图视觉精修）

### 优化

- 月度分布改为轻盈窄柱，增加月份之间的留白并降低非峰值柱的视觉重量
- 增加柔和顶部圆角与统一基线，使月份、数值和柱体对齐更稳定
- 保持主题色映射与移动端最小可读宽度

---

## v2.4.0-beta.4（2026-08-10 · 数据报告兼容性修复）

### 修复

- 移除折线圆点内部的 SVG 提示节点，修复小时图末端中断渲染的问题
- 为数据报告各模块增加独立错误边界，单个图表异常不再阻断后续内容
- 修复 SVG 默认等比缩放造成的桌面端横向留白

### 优化

- 桌面端节律图直接填满报告内容宽度，移动端继续保留横向滚动

---

## v2.4.0-beta.3（2026-08-10 · 数据报告节律图修复）

### 修复

- 修复小时节律图在部分 Obsidian 环境中因 SVG 文字节点而中断渲染的问题
- 恢复星期分布、亮点统计与标签云等节律图后续模块

### 优化

- 小时节律图改为整行宽度，补充高峰时段、峰值笔记、占比、数量刻度与时间轴
- 星期分布改为独立整行展示，避免与小时图争抢宽度

---

## v2.4.0-beta.2（2026-08-10 · 主题色与数据节律预览）

### 新增

- 设置页新增“专注 / 均衡 / 宽阔”三档桌面内容宽度，切换后即时作用于输入区与笔记流
- 数据报告新增星期分布，以主题色圆点面积呈现一周记录节奏

### 优化

- 宠物五维属性条改为跟随 Obsidian 主题色，减少灰暗感
- 发送按钮适当加宽，增强主要操作的视觉落点
- 24 小时记录分布由柱状图改为细线节律图，与月度柱状图形成清晰区分
- 数据报告继续使用原生 DOM 与 SVG，保持离线、主题和移动端兼容性

---

## v2.4.0-beta.1（2026-08-10 · 视觉基础精修预览）

### 优化

- 宽屏下让输入区与笔记流共享居中的内容宽度，减少超长正文行，同时保持顶部工具栏全宽
- 提升时间、总数、统计标签和顶部操作在浅色主题中的必要对比度
- 为主要控件补充统一的主题色键盘焦点环
- 增加 `prefers-reduced-motion` 适配，不影响默认的输入框展开动画
- 建立卡片、面板、控件与胶囊的语义圆角变量，为后续界面统一打底

---

## v2.3.9（2026-07-30 · iOS 快捷记录）

### 新增

- 新增 `obsidian://memoria-capture` 协议，可通过 iOS 快捷指令直接写入文字闪念
- 设置页可复制绑定当前仓库的 URI 模板，并提供中英文配置说明

### 优化

- 手机端未携带文字的快捷指令与「快速记录」命令会展开现有 FAB 输入卡片，与右下角 ➕ 使用相同的键盘、安全区和发送体验
- 桌面端继续使用快速记录弹窗，并保留 `Ctrl+Enter` 与 `Esc` 快捷键提示
- 完成三轮 GitHub Prerelease 与 iOS 真机测试后转为正式版本
